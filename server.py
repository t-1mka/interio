...
FastAPI + SQLite + GigaChat AI + Telegram + Email
"""
...
# GigaChat — многофункциональный ИИ-советник
...
# Широкий набор слов — дизайн, интерьер, бюджет, комнаты, размеры
DESIGN_WORDS = [
    "дизайн", "интерьер", "ремонт", "стиль", "квартир", "комнат", "помещен",
    "мебел", "отделк", "планировк", "освещен", "цвет", "бюджет", "стоимост",
    "площад", "размер", "метр", "кухн", "спальн", "гостин", "ванн", "детск",
    "прихож", "кабинет", "балкон", "лодж", "коридор", "кладов", "гардероб",
    "лофт", "минимализм", "скандинав", "классик", "современн", "прованс",
    "штор", "пол", "потолок", "стен", "плитк", "ламинат", "паркет",
    "диван", "стол", "стул", "шкаф", "хранен", "декор", "текстил",
    "свет", "лампа", "люстр", "торшер", "подсветк",
    "сантехник", "труб", "электрик", "проводк", "розетк",
    "заказ", "срок", "материал", "ткань", "кожа", "дерево", "металл",
    "interior", "design", "renovation", "furniture", "budget", "room", "apartment"
]

async def gc_ask(question):
    """Многофункциональный ИИ-советник по интерьеру, дизайну, бюджету, комнатам"""
    q = question.lower()
    if not any(w in q for w in DESIGN_WORDS):
        return "🏠 Я — советник по интерьеру и дизайну. Могу помочь с:\n\n• Выбором стиля интерьера\n• Планировкой комнат\n• Подбором бюджета на ремонт\n• Размерами помещения\n• Выбором мебели и материалов\n• Цветовой гаммой\n\nСпросите что-нибудь по этим темам!"

    tok = await gc_get_token()
    if not tok: return "🤖 ИИ временно недоступен. Попробуйте позже."

    prompt = (
        "Ты — профессиональный дизайнер интерьера и консультант по ремонту. "
        "Ты разбираешься в стилях интерьера, планировке комнат, расчёте бюджета, "
        "подборе материалов и мебели, размерах помещений, освещении, цветовой гамме. "
        "Ответь кратко (2-4 предложения) на русском языке на вопрос пользователя. "
        "Если вопрос про бюджет — дай примерную оценку. "
        "Если про размеры — дай рекомендации по оптимальной площади. "
        "Если про стиль — опиши его особенности. "
        f"Вопрос: {question}"
    )
    ...
    try:
        async with httpx.AsyncClient(verify=False, timeout=20) as c:
            ...
            if not choices: return "⚠️ ИИ не ответил"
            return choices[0]["message"]["content"].strip()
    except httpx.TimeoutException: return "⏱ Превышено время ожидания"
    except Exception as e: return f"⚠️ Ошибка: {type(e).__name__}"
...
async def notify_email(data):
    ...
    msg = EmailMessage()
    msg["Subject"] = f"🏠 Заявка от {data.get('name', '')}"
    msg["From"] = SMTP_USER; msg["To"] = ADMIN_EMAIL
    msg.add_alternative(
        f"<html><body><h2>Новая заявка Interio</h2>"
        f"<p><b>Имя:</b> {data.get('name')}</p><p><b>Телефон:</b> {data.get('phone')}</p>"
        f"<p><b>Email:</b> {data.get('email', '—')}</p><p><b>Помещение:</b> {data.get('room_type')}</p>"
        f"<p><b>Стиль:</b> {data.get('style')}</p><p><b>Бюджет:</b> {data.get('budget')}</p>"
        f"<p><b>Площадь:</b> {data.get('area', '—')} м²</p><p><b>Комментарий:</b> {data.get('comment', '—')}</p>"
...
async def quiz_submit(req: QuizSubmit):
    ...
    u = c.fetchone(); uid = u["id"] if u else None
...
async def quiz_all():
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT * FROM quiz_submissions ORDER BY created_at DESC LIMIT 50")
...
async def portfolio_like(did: int, req: Request):
    ...
    c.execute("SELECT id FROM likes WHERE design_id = ? AND ip_address = ?", (did, ip))
    existing = c.fetchone()
    if existing:
...
async def portfolio_publish(req: PortfolioPublish):
    pj = json.dumps(req.scene_params, ensure_ascii=False)
    conn = get_db(); c = conn.cursor()
    c.execute('''INSERT INTO portfolio (title, description, scene_params, style, room, budget) VALUES (?,?,?,?,?,?)''',
        (req.title, req.description, pj, req.scene_params.get("style",""), req.scene_params.get("room_type",""), req.scene_params.get("budget","")))
...
async def track_request(req: PhoneCheck):
    ...
    print(f"📱 Код для {req.phone}: {code}")
