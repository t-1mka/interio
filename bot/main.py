...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "👋 Добро пожаловать в <b>СвойСтиль</b>!\n\n"
        "🏠 Создайте дизайн-проект мечты за 6 шагов.\n"
        "🎨 Получите ИИ-генерацию, PDF и промокод.",
...
async def quiz_style(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "Напишите ваш бюджет в формате:\n"
        "<code>мин макс</code> (тыс. руб.)\n\n"
        "Например: <code>200 500</code>",
        parse_mode="HTML",
    )
...
async def quiz_budget(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("❌ Введите два числа через пробел, например: <code>200 500</code>", parse_mode="HTML")
...
async def quiz_colors_done(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "Напишите ваши пожелания к дизайну (или отправьте «нет»):",
        parse_mode="HTML",
    )
...
async def main():
    logger.info("🤖 СвойСтиль Bot starting...")

    # Запускаем HTTP-сервер для health check (нужен для Render)
    async def health_handler(request):
        return web.Response(
            text=json.dumps({"status": "ok", "bot": "СвойСтиль"}),
